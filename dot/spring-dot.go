...
func (gc *GraphContext) buildCommonNode(i int, beanRealType reflect.Type) {
	...
	for j := 0; j < beanRealType.NumField(); j++ {
		...
		if hasTag {
			...
			nodes[j + 1].label = field.Name + ":" + fieldRealType.String()
	...
	//节点太多做个分组
	if beanRealType.NumField() >= RankValue * 2 {
		rank := make([]Rank, cast.ToInt(math.Ceil(float64(beanRealType.NumField())/RankValue)))
		for k := 1; k < len(nodes); k++ {
			rank[k % len(rank)].nodeNames = append(rank[k % len(rank)].nodeNames, nodes[k].name)
		}
		gc.graphs[i].rank = rank
	}
	gc.graphs[i].nodes = nodes
}

func (gc *GraphContext) buildArrayNode(i int) {
	gc.graphs[i].nodes = []Node{{
		name:  gc.beans[i].Name(),
		shape: NodeShapeRecord,
		label: collectionLabel(gc.beans[i].TypeName(), gc.beans[i].Value().Len()),
	}}
}
...
func collectionLabel(name string, len int) string {
	label := fmt.Sprintf("%s\\n|length:%d", name, len)
...
func (graph *SubGraph) writeDot(buffer *bytes.Buffer, offset int) {
	...
	if len(graph.rank) > 0 {
		...
		builder.Write([]byte(fmt.Sprintf("\"%s\"", graph.rank[0].nodeNames[0])))
		for _, rank := range graph.rank[1:] {
			rank.writeDot(buffer, offset + 2)
			builder.Write([]byte(fmt.Sprintf("->\"%s\"", rank.nodeNames[0])))
		}
		builder.Write([]byte("[style=invis]"))
		builder.Write([]byte("\n"))
		writeWithOffset(buffer, offset + 2, builder.String())
	}
...
func (rank *Rank) writeDot(buffer *bytes.Buffer, offset int) {
	builder := strings.Builder{}
	builder.Write([]byte("{rank = same;"))
	for _, name := range rank.nodeNames {
		builder.Write([]byte(fmt.Sprintf("\"%s\";", name)))
	}
	builder.Write([]byte("}\n"))
	writeWithOffset(buffer, offset, builder.String())
}

func (edge *Edge) writeDot(buffer *bytes.Buffer, offset int) {
	if edge.hidden {
		writeWithOffset(buffer, offset, fmt.Sprintf("\"%s\" -> \"%s\"[style=invis]\n", edge.fromName, edge.toName))
	} else {
